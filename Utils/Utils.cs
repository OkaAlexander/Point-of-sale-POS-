...
namespace POS.Utils
{
...

    {
        ...
        /// <returns></returns>
        public ServicesModel PrepareServiceModel(ListViewItem item, string date,string serviceid)
        {
            ServicesModel serviceModel = new ServicesModel();
            serviceModel.ProductId = item.SubItems[0].Text.ToString();
            serviceModel.ProductUnitCost = item.SubItems[3].Text.ToString();
            serviceModel.ProductName = item.SubItems[1].Text.ToString();
            serviceModel.QuantitySold = item.SubItems[2].Text.ToString();
            serviceModel.ServiceDate = date;
            serviceModel.ServiceId = serviceid;
            serviceModel.ServiceTotalCost = item.SubItems[4].Text.ToString();
            return serviceModel;
        }
