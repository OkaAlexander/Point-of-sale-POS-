...
namespace POS.Views
{
    public partial class MainPageForm :MaterialSkin.Controls.MaterialForm
    {
        ...
        {
            SelectedProductsList.View = View.Details;
            try
            {
                ...
                CurrentlyAddedListItem = addedItem.Index.ToString();
            }
            catch (Exception ex)
            {
        ...
        private void RemoveSelectedProductbtn_Click(object sender, EventArgs e)
        {
            if (CurrentlyAddedListItem != null)
            {
                if (SelectedProductsList.Items.Count > 0)
                {
                    ...
                    {
                        if (item.Index == Int32.Parse(CurrentlyAddedListItem))
                        {
                            removeItemModel.ItemId=item.SubItems[1].Text;
                            removeItemModel.ItemQuantity=item.SubItems[2].Text;
                            PrepareTransactionCost(item.SubItems[4].Text,"remove");
                           // MessageBox.Show(removeItemModel.ItemQuantity, "test", MessageBoxButtons.OK, MessageBoxIcon.Information);
                           
                        }
                    }
                    SelectedProductsList.Items.RemoveAt(Int32.Parse(CurrentlyAddedListItem));
